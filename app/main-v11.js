...
// Scenario practice: each event is framed as a question; every decision gets
// immediate coaching feedback stored in state.eventFeedback (persisted through
// normalizeState's top-level spread, like the other run-scoped keys).

function eventQuestion(e){
  if(e.question) return e.question;
  if(e.type==='info') return `What should you note about \u201c${e.title}\u201d before deciding anything else?`;
  if(e.id==='save'||e.type==='save') return 'After required costs, what should move to savings this period?';
  const amt=e.amount?` costs ${money(e.amount)}`:'';
  return `${e.title}${amt} \u2014 what do you do?`;
}

function eventLabel(e){
  return e.type==='need'?'Need':e.type==='want'?'Want':e.type==='save'?'Savings':e.type==='info'?'Information':'Recurring';
}

function payFeedback(e,type,amount,coherent,shortfall){
  const amt=money(amount);
  const warn=shortfall>0?` This leaves ${money(shortfall)} of required costs uncovered \u2014 check \u201cKnown Needs left\u201d before the next decision.`:'';
  if(type==='want') return coherent
    ? `Spent ${amt} on a want \u2014 and required costs are still covered. A planned want is not a mistake.`
    : `Spent ${amt} on a want, but ${money(shortfall)} of required costs are now uncovered. Wants wait for what\u2019s left \u2014 check needs first next time.`;
  if(type==='recurring') return `Paid ${amt} \u2014 recurring costs belong to the plan even when they charge on another day.`+warn;
  return `Paid ${amt} \u2014 required costs are protected first. That\u2019s the routine working.`+warn;
}

function renderEvent(e){
  const teach=state.profile.scaffold==='Teach', guide=state.profile.scaffold==='Guide', revealed=state.revealedHints[e.id];
  ...
  if(!state.weekStarted) controls='<span class="sub">Start the scenario first.</span>';
    ...
    const payLabel=e.required?`Pay ${money(e.amount)} (protect it)`:e.type==='recurring'?`Pay ${money(e.amount)} (recurring)`:e.type==='want'?`Spend ${money(e.amount)}`:`Pay ${money(e.amount)}`;
    const skipLabel=e.required?'':e.type==='recurring'?'Pause / skip':'Skip for now';
    controls=`<div class="row"><button class="btn" onclick="app.payEvent('${e.id}',${e.amount},'${e.type}','${esc(e.title)}')">${payLabel}</button>${skipLabel?`<button class="btn secondary" onclick="app.skipEvent('${e.id}')">${skipLabel}</button>`:''}</div>`;
  }
  ...
  return `<div class="event"><div class="row between"><div><span class="tag ${e.type}">${eventLabel(e)}</span><h3 style="margin:7px 0 3px">${esc(e.title)}</h3></div><b>${e.type==='info'?'Information':e.id==='save'?'Suggested '+money(e.amount):money(e.amount)}</b></div><p class="event-question"><b>${esc(eventQuestion(e))}</b></p><p>${esc(e.text||'')}</p>${state.preferences.wording==='standard'?support:''}<div style="height:10px"></div>${controls}</div>`;
}

function renderDecidedEvent(e){
  const fb=state.eventFeedback?.['event:'+e.id]||{};
  return `<div class="event done"><div class="row between"><div><span class="tag ${e.type}">${eventLabel(e)}</span><b>${esc(e.title)}</b></div><span class="positive">${esc(fb.decision||'Decided')}</span></div>${fb.line?`<p class="sub">${esc(fb.line)}</p>`:''}</div>`;
}

function renderWeek(){
  const list=events();
  const doneCount=list.filter(e=>state.completed['event:'+e.id]).length;
  const pending=list.filter(e=>!state.completed['event:'+e.id]);
  const handleFirst=pending.filter(e=>e.required||e.type==='recurring');
  const yourCall=pending.filter(e=>!(e.required||e.type==='recurring'));
  const decided=list.filter(e=>state.completed['event:'+e.id]);
  const progress=doneCount<list.length?`Decision ${doneCount+1} of ${list.length}`:`All ${list.length} decisions made`;
  const period=state.activeResponsibilityPeriod ? ` Responsibility-transfer period ${state.activeResponsibilityPeriod}.` : '';
  $('week').innerHTML=`<div class="hero"><div class="row between"><div><h2>Scenario Practice</h2><p class="sub">Seed ${esc(state.profile.scenarioSeed)} reproduces the same event sequence. Mode: ${esc(state.profile.scenarioMode)}.${period}</p></div>${state.weekStarted?'<button class="btn secondary" onclick="app.finishWeek()">End & review</button>':'<button class="btn" onclick="app.startWeek()">Start scenario</button>'}</div>${state.weekStarted?`<div class="stats"><div class="stat"><span>Started with</span><b>${money(state.currentRunStart)}</b></div><div class="stat"><span>Balance now</span><b>${money(state.profile.balance)}</b></div><div class="stat"><span>Hints used</span><b>${Math.max(0,state.learning.hintsUsed-(state.currentRunHintsStart||0))}</b></div><div class="stat"><span>Known Needs left</span><b>${money(outstanding())}</b></div></div>`:''}</div><div class="section-title"><div><h2>Decisions</h2><p>${progress} \u2014 ${state.profile.scaffold==='Teach'?'worked reasoning is visible.':state.profile.scaffold==='Guide'?'use cues first; request a step only when needed.':'no automatic solution cues. Help remains available.'}</p></div></div>${handleFirst.length?`<div class="section-title"><div><h2>Handle first</h2><p>These are due before anything optional.</p></div></div><div class="stack">${handleFirst.map(renderEvent).join('')}</div>`:''}${yourCall.length?`<div class="section-title"><div><h2>Your call</h2><p>Optional. Decide with what\u2019s left.</p></div></div><div class="stack">${yourCall.map(renderEvent).join('')}</div>`:''}${decided.length?`<div class="section-title"><div><h2>Decided</h2><p>What you chose \u2014 and what it means.</p></div></div><div class="stack">${decided.map(renderDecidedEvent).join('')}</div>`:''}`;
}

function renderSpend(){
  const transfer=state.transferMode;
  const q=[
    {id:'q1',skill:'needs',text:`You have ${money(weekly())}. Transportation of ${money(scaledAmount(state.profile,10))} is still required. A friend asks you to spend ${money(scaledAmount(state.profile,18))} now.`,good:'plan',choices:[['spend','Spend now \u2014 friends come first'],['plan','Check required costs first'],['borrow','Borrow money to cover both'],['halve','Spend half now and half later']],ex:'Checking required costs first protects money you already know is needed. Borrowing adds a future cost, and splitting the spend still risks the required amount.'},
    {id:'q2',skill:'sales',text:`A ${money(scaledAmount(state.profile,30))} item is 30% off, but it was not planned.`,good:'wait',choices:[['sale','Buy it \u2014 30% off is too good to pass up'],['wait','Check the plan and required costs first'],['two','Buy two while the discount lasts'],['later','Wait for an even bigger discount']],ex:'A sale changes the price, not whether the purchase fits the plan. Stockpiling an unplanned item just spends more money, not less.'},
    {id:'q3',skill:'needs',text:'Prepared food is necessary twice each week because cooking is not realistically accessible on those days. How should it be classified?',good:'depends',choices:[['need','Always a Need'],['want','Always a Want'],['depends','It depends on the situation'],['income','It depends only on income']],ex:'Needs and Wants are contextual. Ask what function the expense serves and what alternatives exist \u2014 the label follows the situation, not the price tag.'},
    {id:'q4',skill:'sales',text:'A bulk pack costs less per unit, but half of it would expire before you use it. The smaller pack costs more per unit, but you would use all of it.',good:'small',choices:[['bulk','Buy the bulk pack \u2014 the unit price is lower'],['small','Buy the smaller pack \u2014 you will use all of it'],['neither','Buy neither'],['both','Buy one of each']],ex:'Value is what you actually use, not the unit price on the shelf. The \u201ccheaper\u201d pack costs more when half of it is wasted.'}
  ];
...
function retrievalChallenge(check){
  const statement=createFictionalPaycheck();
  const grossMinusOne=+(statement.gross-(statement.deductions[0]?.amount||0)).toFixed(2);
  const map={
    weekly:{question:'Transportation costs $12 before your next refill. A $14 movie is optional. What should you protect first?',choices:[['protect','Protect transportation'],['movie','Buy the movie first'],['split','Split the money between both'],['saveall','Save it all and skip both']],good:'protect',help:'Look at what must happen before the next income event.'},
    needs:{question:'Prepared food is required on a day when cooking is not realistically accessible. How should NWS classify it?',choices:[['need','Always a Need'],['want','Always a Want'],['depends','It depends on the situation'],['income','It depends only on income']],good:'depends',help:'Needs and Wants are contextual. Ask what function the expense serves and what alternatives exist.'},
    saving:{question:'A goal needs $100 more and there are 5 periods left. What is the needed contribution per period?',choices:[['10','$10'],['20','$20'],['25','$25'],['40','$40']],good:'20',help:'Divide the amount still needed by the periods remaining.'},
    recurring:{question:'A service costs $10 each month. What is its approximate yearly cost?',choices:[['100','$100'],['120','$120'],['140','$140'],['200','$200']],good:'120',help:'Annualize a monthly recurring cost by multiplying by 12.'},
    sales:{question:'An unplanned item is discounted. What should you check before buying it?',choices:[['sale','The discount alone'],['plan','Your plan, required costs, and goals'],['brand','The brand name'],['reviews','Online reviews']],good:'plan',help:'A sale changes the price, not automatically whether the purchase fits your plan.'},
    semester:{question:'You have $600 for 15 weeks and must reserve $120 for travel. About how much is available per week after reserving travel?',choices:[['32','$32'],['40','$40'],['48','$48'],['28','$28']],good:'32',help:'Reserve the known future cost first, then divide what remains by the weeks.'},
    income:{question:`A fictional paycheck shows gross ${money(statement.gross)} and take-home ${money(statement.takeHome)}. Which amount is available to budget?`,choices:[[String(statement.gross),money(statement.gross)],[String(statement.deductionTotal),money(statement.deductionTotal)],[String(statement.takeHome),money(statement.takeHome)],[String(grossMinusOne),money(grossMinusOne)]],good:String(statement.takeHome),help:'Use the amount that actually reaches the worker after deductions: take-home pay.'}
  };
...
// Course completion — the same visited set as the course home (localStorage
// nwsCourseShell.v1 via readCourseState), shown with per-module breakdowns and
// a next-step recommendation. Visits are navigation history, not a mastery
// score — the non-evaluative framing is required everywhere progress appears.
function renderCourseCompletion(){
  const visited=readCourseState().visited||{};
  const all=MODULES.flatMap(module=>module.lessons);
  const total=all.length;
  const done=all.filter(lesson=>visited[lesson.id]).length;
  const pctDone=total?Math.round(done/total*100):0;
  const next=all.find(lesson=>!visited[lesson.id]);
  const nextModule=next?MODULES.find(module=>module.lessons.some(lesson=>lesson.id===next.id)):null;
  const rows=MODULES.map(module=>{
    const seen=module.lessons.filter(lesson=>visited[lesson.id]);
    const minsDone=seen.reduce((sum,lesson)=>sum+(lesson.est||0),0);
    const minsTotal=moduleMinutes(module);
    const modulePct=module.lessons.length?Math.round(seen.length/module.lessons.length*100):0;
    const nextStep=module.lessons.find(lesson=>!visited[lesson.id]);
    return `<div class="pmod"><div class="pmod-head"><b>Module ${module.number}: ${esc(module.title)}</b><span class="tag">${seen.length}/${module.lessons.length} steps</span></div><div class="progressbar" role="progressbar" aria-valuemin="0" aria-valuemax="${module.lessons.length}" aria-valuenow="${seen.length}" aria-label="Module ${module.number} steps visited"><i style="width:${modulePct}%"></i></div><p class="sub">${minsDone} of ${minsTotal} minutes visited${nextStep?` · Next: <button type="button" class="linklike" onclick="course.openLesson('${esc(nextStep.id)}')">${esc(nextStep.title)}</button>`:''}</p></div>`;
  }).join('');
  return `<div class="section-title"><div><h2>Course completion</h2><p>Steps you have visited across the 6 modules.</p></div></div><div class="card"><div class="course-progress-head"><h3>Overall</h3><span class="tag">${done} of ${total} steps</span></div><div class="progressbar" role="progressbar" aria-valuemin="0" aria-valuemax="${total}" aria-valuenow="${done}" aria-label="Course steps visited"><i style="width:${pctDone}%"></i></div><p class="sub">Visits are shown for navigation only — they are not a mastery score.</p><div class="stack pmod-list">${rows}</div>${next?`<div class="row course-cta-row"><button class="btn" type="button" onclick="course.openLesson('${esc(next.id)}')">Next up: ${esc(next.title)}</button><span class="sub">${next.est} min${nextModule?` · Module ${nextModule.number}: ${esc(nextModule.title)}`:''}</span></div>`:'<p class="positive">You have visited every step — review anything, any time.</p>'}</div>`;
}

function renderProgress(){
  const report=generateInstructorReport(state);
  $('progress').innerHTML=`<div class="hero"><div class="row between"><div><h2>Progress</h2><p class="sub">One quiz doesn't tell the whole story.</p></div><div class="row"><button class="btn secondary" onclick="app.previewInstructorReport()">Preview advisor report</button><button class="btn secondary" onclick="app.downloadInstructorReport()">Download report</button></div></div><div class="stats"><div class="stat"><span>Scenario attempts</span><b>${state.attempts}</b></div><div class="stat"><span>Hints requested</span><b>${state.learning.hintsUsed}</b></div><div class="stat"><span>Recorded decisions</span><b>${state.learning.decisions.length}</b></div><div class="stat"><span>Current support</span><b>${esc(state.profile.scaffold)}</b></div></div></div>${renderCourseCompletion()}${renderRetrieval()}<div class="section-title"><h2>How you're doing</h2></div><div class="card"><div style="overflow-x:auto"><table><thead><tr><th>Skill</th><th>Correct answers</th><th>Without hints</th><th>In new situations</th><th>Remembered later</th><th>After mistakes</th></tr></thead><tbody>${SKILLS.map(s=>{const x=getSkillStats(state,s.id);return `<tr><td>${esc(s.label)}</td><td>${pct(x.accuracy)}</td><td>${pct(x.independence)}</td><td>${pct(x.transfer)}</td><td>${esc(x.retention)}</td><td>${pct(x.recovery)}</td></tr>`}).join('')}</tbody></table></div><p class="sub">These show patterns over time — not a grade. There is no pass or fail here.</p></div><div class="section-title"><h2>Current advisor summary</h2></div><div class="card"><div class="stats"><div class="stat"><span>Scored decisions</span><b>${report.summary.scoredDecisions}</b></div><div class="stat"><span>Correct answers</span><b>${pct(report.summary.accuracyPercent)}</b></div><div class="stat"><span>Without hints</span><b>${pct(report.summary.independencePercent)}</b></div><div class="stat"><span>Checks due</span><b>${report.summary.delayedChecksDue}</b></div></div><p class="sub">${esc(report.interpretationNote)}</p></div>`;
}
...
function scenarioReviewSections(list){
  const fb=id=>state.eventFeedback?.['event:'+id]||{};
  const recap=list.map(e=>{
    const f=fb(e.id);
    return `<div class="decision-recap"><b>${esc(e.title)}</b> \u2014 ${esc(f.decision||'No decision recorded')}${f.line?`<br><span class="sub">${esc(f.line)}</span>`:''}</div>`;
  }).join('');
  const requiredEvents=list.filter(e=>e.required||e.type==='need');
  const requiredCovered=requiredEvents.filter(e=>/^(Paid|Spent)/.test(fb(e.id).decision||''));
  const risky=list.filter(e=>fb(e.id).risky).length;
  const hintsUsed=Math.max(0,state.learning.hintsUsed-(state.currentRunHintsStart||0));
  const saved=savedInLedger(state.ledger);
  const well=[];
  if(requiredEvents.length&&requiredCovered.length===requiredEvents.length) well.push('Every required cost was protected before optional spending.');
  if(saved>0) well.push(`${money(saved)} was set aside for later.`);
  if(hintsUsed===0) well.push('Decisions were made without hints.');
  const next=[];
  if(risky>0) next.push(`${risky} decision${risky>1?'s':''} left required costs uncovered \u2014 pause and check \u201cKnown Needs left\u201d first.`);
  if(!(saved>0)) next.push('Try moving a small amount to savings earlier in the run.');
  return {recap,
    wellHtml:well.length?`<h3>What went well</h3><div class="stack">${well.map(w=>`<div class="callout">${esc(w)}</div>`).join('')}</div>`:'',
    nextHtml:next.length?`<h3>Try next time</h3><div class="stack">${next.map(w=>`<div class="callout">${esc(w)}</div>`).join('')}</div>`:''};
}

window.app={
  show,
  preset(cadence,amount){Object.assign(state.profile,{cadence,amount,balance:amount});state.ledger=[];state.completed={};state.eventFeedback={};state.weekStarted=false;state.currentRunStart=null;state.currentScenarioEvents=[];persist();renderAll()},
  saveSetup(){const g=id=>$(id);Object.assign(state.profile,{name:g('pName').value||'Student',cadence:g('pCadence').value,amount:+g('pAmount').value||0,balance:+g('pBalance').value||0,savings:+g('pSavings').value||0,scaffold:g('pScaffold').value,scenarioMode:g('pScenario').value,scenarioSeed:g('pSeed').value||'NWS-001',emergencyTarget:+g('pEmergency').value||0,shortGoal:+g('pShort').value||0,mediumGoal:+g('pMedium').value||0,longGoal:+g('pLong').value||0});state.currentScenarioEvents=[];state.eventFeedback={};state.activeResponsibilityPeriod=null;persist();show('home')},
  ...
  startWeek(){state.attempts++;state.weekStarted=true;state.ledger=[];state.completed={};state.eventFeedback={};state.revealedHints={};state.currentRunHintsStart=state.learning.hintsUsed;state.currentRunStart=scenarioStartAmount(state);state.profile.balance=state.currentRunStart;state.currentScenarioEvents=buildScenarioEvents(state);state.activeResponsibilityPeriod=null;record('weekly',null,{detail:'Scenario started'});persist();renderAll()},
  startResponsibilityPeriod(periodNumber){const period=buildResponsibilityTransferSequence(state).find(x=>x.period===periodNumber);if(!period)return;state.attempts++;state.profile.scaffold=period.scaffold;state.profile.scenarioMode='responsibility';state.profile.scenarioSeed=period.seed;state.transferMode=period.transfer;state.activeResponsibilityPeriod=period.period;state.weekStarted=true;state.ledger=[];state.completed={};state.eventFeedback={};state.revealedHints={};state.currentRunHintsStart=state.learning.hintsUsed;state.currentRunStart=weekly();state.profile.balance=state.currentRunStart;state.currentScenarioEvents=[{id:`period-${period.period}-info`,title:period.title,amount:0,type:'info',text:period.text,required:false},...period.obligations.map(o=>({id:o.id,title:o.title,amount:o.amount,type:'need',text:`${o.title} is required in this practice period.`,required:true})),{id:`period-${period.period}-want`,title:'Flexible choice',amount:scaledAmount(state.profile,10),type:'want',text:'An optional purchase is available. Check required costs first.',required:false},{id:'save',title:'Choose an amount to save',amount:Math.max(1,Math.round(weekly()*.10)),type:'save',text:'Choose a savings amount after checking required costs.',required:false}];record('weekly',null,{detail:`Responsibility period ${period.period} started`});persist();show('week')},
  payEvent(id,amount,type,title){const e=events().find(x=>x.id===id);const after=state.profile.balance-amount,needsAfter=Math.max(0,outstanding()-(type==='need'?amount:0));if(after<0&&!confirm('This choice would take the simulated balance below zero. Record it anyway for learning?'))return;state.ledger.push({id:Date.now()+Math.random(),desc:title,amount:-amount,type,date:new Date().toISOString()});state.profile.balance=+after.toFixed(2);state.completed['event:'+id]=true;const coherent=after>=needsAfter;const shortfall=Math.max(0,outstanding()-state.profile.balance);state.eventFeedback ||= {};state.eventFeedback['event:'+id]={decision:(type==='want'?'Spent ':'Paid ')+money(amount),line:payFeedback(e||{id,title,amount,type},type,amount,coherent,shortfall),risky:!coherent};record(skillForTransactionType(type),coherent,{prompted:!!state.revealedHints[id],transfer:!!state.transferMode,errorType:coherent?'':'planning',detail:`${title}: ${type}`});persist();renderAll()},
  reviewEvent(id,skill='weekly'){state.completed['event:'+id]=true;state.eventFeedback ||= {};state.eventFeedback['event:'+id]={decision:'Reviewed',line:'Reviewed \u2014 noted for the plan. Information first, decisions second.',risky:false};record(skill,true,{transfer:!!state.transferMode,detail:'Information reviewed'});persist();renderAll()},
  skipEvent(id){const e=events().find(x=>x.id===id);state.completed['event:'+id]=true;state.eventFeedback ||= {};state.eventFeedback['event:'+id]={decision:'Skipped',line:e&&e.required?'Skipped a required cost \u2014 it stays due. Cover it before optional spending.':'Skipped \u2014 nothing lost. Optional choices wait for what\u2019s left.',risky:!!(e&&e.required)};record(id==='save'?'saving':'weekly',true,{prompted:!!state.revealedHints[id],transfer:!!state.transferMode,detail:'Skipped or delayed optional choice'});persist();renderAll()},
  ...
  saveFromWeek(){const n=+$('saveWeekAmt').value||0;if(n>state.profile.balance)return alert('Choose a smaller amount.');const coherent=(state.profile.balance-n)>=outstanding();state.profile.balance=+(state.profile.balance-n).toFixed(2);state.profile.savings=+(state.profile.savings+n).toFixed(2);state.ledger.push({id:Date.now(),desc:'Savings transfer',amount:n,type:'save',date:new Date().toISOString()});state.completed['event:save']=true;const shortfall=Math.max(0,outstanding()-state.profile.balance);state.eventFeedback ||= {};state.eventFeedback['event:save']={decision:'Saved '+money(n),line:coherent?`Saved ${money(n)} \u2014 moved to later before flexible spending, with required costs still covered.`:`Saved ${money(n)}, but ${money(shortfall)} of required costs are now uncovered. Protect needs first, then save from what\u2019s left.`,risky:!coherent};record('saving',coherent,{prompted:!!state.revealedHints.save,transfer:!!state.transferMode,errorType:coherent?'':'planning',detail:`Saved ${money(n)}`});persist();renderAll()},
  finishWeek(){const body=$('reportBody'),needs=spentByType(state.ledger,'need'),wants=spentByType(state.ledger,'want'),rec=spentByType(state.ledger,'recurring'),saved=savedInLedger(state.ledger),protectedNeeds=outstanding()<=state.profile.balance;state.report={needs,wants,rec,saved,remaining:state.profile.balance,protectedNeeds};const review=scenarioReviewSections(events());record('weekly',state.profile.balance>=0&&protectedNeeds,{transfer:!!state.transferMode,errorType:protectedNeeds?'':'planning',detail:'End-of-scenario review'});addScenarioHistory({protectedNeeds});body.innerHTML=`<h2>Scenario review</h2><p class="sub">This describes consequences, not a moral grade.</p><h3>Decision recap</h3><div class="stack">${review.recap}</div>${review.wellHtml}${review.nextHtml}<h3>Numbers</h3><table><tbody><tr><td>Starting amount</td><td>${money(state.currentRunStart||weekly())}</td></tr><tr><td>Needs paid</td><td>${money(needs)}</td></tr><tr><td>Wants</td><td>${money(wants)}</td></tr><tr><td>Recurring</td><td>${money(rec)}</td></tr><tr><td>Saved</td><td>${money(saved)}</td></tr><tr><td>Remaining</td><td>${money(state.profile.balance)}</td></tr></tbody></table><p>${protectedNeeds?'Known required costs remain covered.':'At least one known required cost is not fully protected. What could change, move, or wait?'}</p><div class="row"><button class="btn" onclick="app.closeReport()">Continue</button><button class="btn secondary" onclick="app.replayWeek()">Replay same seed</button></div>`;persist();$('reportDialog').showModal()},
  closeReport(){$('reportDialog').close();show(state.activeResponsibilityPeriod?'plan':'progress')},
  replayWeek(){$('reportDialog').close();state.weekStarted=false;state.ledger=[];state.completed={};state.eventFeedback={};state.revealedHints={};state.profile.balance=weekly();state.currentRunStart=null;persist();show('week')},
